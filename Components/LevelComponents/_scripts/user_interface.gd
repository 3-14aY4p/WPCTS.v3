...
func _physics_process(delta: float) -> void:
	if task:
		if task.task_state == task.TaskState.ONGOING:
			update_timer_label()
	else: task = GameController.current_task

func update_timer_label():
	if task.task_state == task.TaskState.ONGOING:
		timer_label.show()
		if task.time_limit > 0:
			var m: float = task.timer.time_left/60
			var s: float = fmod(task.timer.time_left, 60)
			if s >= 10:
				timer_label.text = "%d:%d" % [int(m), int(s)]
			else:
				timer_label.text = "%d:0%d" % [int(m), int(s)]
